...
DEBUG ((DEBUG_ERROR,
"%a: %a: failed to allocate OpRegion: %r\n",
__FUNCTION__,
...
if(OpRegionResidual){
ZeroMem (BytePointer
+ mOpRegionSize, OpRegionResidual);
}
...
DEBUG ((DEBUG_ERROR,
"%a: %a: failed to write OpRegion address: %r\n",
__FUNCTION__,
GetPciName (PciInfo), Status
));
goto
...
"i915: %a: OpRegion @ 0x%Lx size 0x%Lx in %d pages\n", __FUNCTION__,
...
FreeOpRegion:
gBS->
FreePages (Address, OpRegionPages
);
return
Status;
}
...
DEBUG ((DEBUG_ERROR,
"%a: %a: failed to allocate stolen memory: %r\n",
__FUNCTION__,
GetPciName (PciInfo), Status
));
return
...
DEBUG ((DEBUG_ERROR,
"%a: %a: failed to write stolen memory address: %r\n",
__FUNCTION__,
GetPciName (PciInfo), Status
));
goto
...
DEBUG ((DEBUG_INFO,
"%a: %a: stolen memory @ 0x%Lx size 0x%Lx\n",
__FUNCTION__,
GetPciName (PciInfo), Address, (UINT64)
mBdsmSize));
return
EFI_SUCCESS;
...
STATIC EFI_STATUS
SetupFwcfgStuff(EFI_PCI_IO_PROTOCOL
*PciIo){
...
//
if (
EFI_ERROR (OpRegionStatus)
&&
EFI_ERROR (BdsmStatus)
) {
return
EFI_UNSUPPORTED;
}
...
Private->
Signature = SIGNATURE_32('i', '9', '1', '5');
...
DebugPrint(EFI_D_ERROR,
"i915: bar ranges - %llx %llx, %llx %llx\n",
...
UINT32 x_active = g_private.edid.detailTimings[DETAIL_TIME_SELCTION].horzActive |
                  ((UINT32)(g_private.edid.detailTimings[DETAIL_TIME_SELCTION].horzActiveBlankMsb >> 4) << 8);
UINT32 y_active = g_private.edid.detailTimings[DETAIL_TIME_SELCTION].vertActive |
...
g_private.
FbBase = aperture_base + (UINT64)(g_private.gmadr);
...
if(!fb_backing){
DebugPrint(EFI_D_ERROR,
"i915: failed to allocate framebuffer\n");
Status = EFI_OUT_OF_RESOURCES;
goto
FreeGopDevicePath;
}
...
DebugPrint(EFI_D_ERROR,
"i915: SetupFwcfgStuff returns %d\n",Status);
}
DebugPrint(EFI_D_ERROR,
...
FirmwareVersion = PcdGet32(PcdDriverSupportedEfiVersion);
Status = gBS->InstallMultipleProtocolInterfaces(
        &ImageHandle,
